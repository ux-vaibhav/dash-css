 ...
 customCss();
  ...
  for (var k = 0; k < allClasses.length; k++) {
    if (allClasses[k].indexOf('-') != -1) {
      var type = allClasses[k].substring(0, allClasses[k].indexOf('-'));
      var val = allClasses[k].substring(allClasses[k].indexOf('-') + 1);
      var device;
      if (allClasses[k].indexOf('_') != -1) {
        device = type.substring(0, allClasses[k].indexOf('_'));
        type = type.substring(allClasses[k].indexOf('_') + 1);
      } else {
        device = undefined;
      }
  ...
  function getSelector(device, type, val) {
    var Selector;
    if (device != undefined) {
      Selector = '.' + device + '_' + type + '-' + val;
    } else {
      Selector = '.' + type + '-' + val;
    }
    return Selector;
  ...
  function addStyle(device, type, val) {
    for (var m = 0; m < allProps.length; m++) {
      if (allProps[m].name.indexOf(type) != -1) {
        var Selector = getSelector(device, type, val);
  ...
  function getValue(val, type, name) {
    ...
    if (name == 'bdr' || name == 'bdrb' || name == 'bdrt' || name == 'bdrl' || name == 'bdrr') {
      newVal = getBorder(newVal);
    }
    if (name == 'cl' || name == 'bg' || name == 'bdrc') {
...
function loadJSON(callback) {
  ...
  xobj.open('GET', 'AllProps.json', true);
